...
function BookAppointment() {
  ...
  let { beauticianId } = useParams();
  const beautician = beauticianList.filter((beautician) => beautician._id == beauticianId);
  let fullname = beautician[0].firstname + " " + beautician[0].lastname;
  const [name, setName] = useState(fullname);
  const [department, setDepartment] = useState(beautician[0].department);
  ...
  const handleSubmit = async (e) => {
    ...
    const values = {
      ...
      beautician_name: fullname,
      beautician_email: beautician[0].email,
      slot: selectedSlot,
      bookedBy: user.email,
    };
      ...
      <div className="appointmentFormDiv">
        ...
        <Form onSubmit={handleSubmit}>
          <Form.Group className="mb-3" controlId="formBasicDoctorName">
            <Form.Label>Beautician Name</Form.Label>
